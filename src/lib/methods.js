...
const upload = async (
  ...
  console.log({ submitedData, fieldName, file });
  ...
  if (file && rawFile && rawFile.name) {
    ...
    result[fieldName] = {};
    result[fieldName].uploadedAt = Date.now();
    const downloadURL = await snapshot.ref.getDownloadURL();
    result[fieldName].src = downloadURL;
    result[fieldName].type = rawFile.type;
...
const getMany = (params, resourceName, resourceData) => {
  let data = Object.values(resourceData).filter(
    item => params.ids.indexOf(item.id) > -1
  );
